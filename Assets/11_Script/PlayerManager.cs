...
public class PlayerManager : MonoBehaviour
{
    ...
    [Header("Move")]
    [SerializeField] bool _canMove;              // 움직임 조건 (스킬 쓰면 못 움직임)
    ...
    [SerializeField] bool _isChange;                 // 기본이 false, 스킬 쓸 때는 true (샷건, 라이플에서 사용)
    ...
    public Player_State currSkill;   // 현재 스킬
    public Player_State preSkilll;    // 전 스킬
    ...
    private void PlayerInit() 
    {
        ...
        // arr[2] : ShotGun 스크립트
        // arr[3] : Lifle 스크립트
        arraySkill[(int)Player_State.Idle]       = new Skill_Idle(this);   // 생성자
    ...
    IEnumerator IE_shootGun() 
    {
        ...
        _trsShootPosi.rotation = Quaternion.Euler(0,0,0);    // shootposi 회전 초기화
        _isChange = true;
    }
    ...
    void GunFire(int gunIdx)
    {
        ...
        obj.transform.position  = _trsShootPosi.position;                    // 발사 위치로 이동
        obj.transform.rotation  = Quaternion.Euler(90, 0,0);                // 회전
        obj.transform.LookAt(_trsShootPosi.transform.forward);               // 총알도 총알 쏘는 위치 보는 방향으로 보게

        obj.GetComponent<Rigidbody>().velocity = _trsShootPosi.forward * myBulletDB.BulletSpeed;     // 현재 Bullet의 속도 가져오기
    }
